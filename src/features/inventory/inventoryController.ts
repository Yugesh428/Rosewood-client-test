...
// Manual stock adjustment: add stock (restock) or subtract (damage/expiry write-off).
// type: "add" | "subtract"
// This is NOT used for order delivery — that's handled in the order flow.

export async function adjustStock(
  req: NextRequest,
  id: string,
): Promise<NextResponse> {
  logger.info(CTX, "adjustStock — start", { id });
  ...
  try {
    ...
    logger.debug(CTX, "adjustStock — payload", { id, type, amount, reason });

    if (!["add", "subtract"].includes(type)) {
      throw new AppError('type must be "add" or "subtract".', 400, "VALIDATION_ERROR");
    }
    const qty = toNum(amount);
    if (qty <= 0) throw new AppError("amount must be a positive number.", 400, "VALIDATION_ERROR");

    const inv = await Inventory.findByPk(id);
    if (!inv) {
      logger.warn(CTX, "adjustStock — not found", { id });
      throw new AppError("Inventory record not found.", 404, "NOT_FOUND");
    }
    ...
    if (type === "add") {
      ...
      if (inv.quantity < qty) {
        throw new AppError(
          `Cannot subtract ${qty} — only ${inv.quantity} in stock.`,
    ...
    logger.info(CTX, "adjustStock — done", {
      id, type, amount: qty,
      before, after: inv.quantity, reason: reason ?? "none",
    });

    return NextResponse.json({
      success: true,
      message: `Stock ${type === "add" ? "added" : "subtracted"}: ${qty} units.`,
      data: {
        id:          inv.id,
    ...
    logger.error(CTX, "adjustStock — failed", { id, error });
    return errorResponse(error);
  }
}
